...
data {
  ...
  matrix <lower = 0> [nEa, nEa] D; //diagonal matrix with the number of neighboors of each EA
...
transformed data {
  ...
  vector[nEa] D_sparse; // diagonal of D (number of neigbouurs for each area)
  vector[nEa] lambda;   // eigenvalues of invsqrtD * W * invsqrtD
  
  { // generate sparse representation for W
  int counter;
  counter = 1;
  // loop over upper triangular part of W to identify neighbour pairs
    for (i in 1:(nEa - 1)) {
      for (j in (i + 1):nEa) {
        if (W[i, j] == 1) {
